...
module Stripe
  class OAuthTest < Test::Unit::TestCase
    ...
    context "StripeOAuthClient" do
      context "#execute_request" do
        context "error handling" do
          should "raise OAuthError on error" do
            stub_request(:post, "#{Stripe.connect_base}/oauth/error").
              to_return(body: JSON.generate({error: 'type', error_description: 'description'}), status: 400)

            client = OAuth::StripeOAuthClient.new
            e = assert_raises(OAuthError) do
              client.execute_request(:post, '/oauth/error')
            end
            assert_equal('type', e.type)
            assert_equal('description', e.message)
          end
        end
      end
    end
